...
| `FoldHeading` | The title of a list that folds away: Obsidian's own fold arrow (a heading's, the file tree's), the title, a muted count. The whole of it is the control — click, Enter or Space emit `toggle`, and the owner passes `collapsed` back. Without `collapsible` it is the plain title. The lists under a note use it. |
