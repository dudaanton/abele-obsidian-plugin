...
| `Card` / `CardGrid` | A repeated item you can click into: a model, an agent, a stored secret. `icon` puts a glyph before the title, `cover` puts a picture across the top and `large` makes the title a heading, for a post in a feed; `thumbnail` puts a small one on the right, for a row in a list. |
