...
| `Breadcrumbs` | The way down to where you are, outermost first: every level but the last pressed to go back to it, each with a tooltip saying what it opens. Wraps rather than scrolls, each level cut to one line. The trail over a comment asked inside a chat. |
