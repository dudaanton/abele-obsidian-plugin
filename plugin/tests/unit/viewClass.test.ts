...
describe('View', () => {
  ...
  it('goes deaf after close: no hooks, no new timers', async () => {
    vi.useFakeTimers()
    const v = new View({ title: 'T' }, host(), origin)
    const key = vi.fn()
    const closed = vi.fn()
  ...
  it('keeps the last twenty errors and does not repeat the last one', () => {
    const v = new View({ title: 'T' }, host(), origin)
    const error = vi.spyOn(console, 'error').mockImplementation(() => {})
    for (let i = 0; i < 30; i++) v.report(new Error('same'))
    expect(v.errors).toEqual(['same'])
    v.dismissErrors()
    for (let i = 0; i < 30; i++) v.report(new Error(`e${i}`))
    expect(v.errors.length).toBe(20)
    expect(v.errors[0]).toBe('e10')
    expect(v.errors[19]).toBe('e29')
    error.mockRestore()
  })
})
