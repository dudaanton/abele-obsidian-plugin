...
import { REMEMBER_TOOL, MEMORY_ITEM_MAX_LENGTH } from '@/ai/agents/memory'
...
function toolOf(session: ChatSession): AgentTool {
  const tools = (session as unknown as { getTools: () => AgentTool[] }).getTools()
  const tool = tools.find((t) => t.name === REMEMBER_TOOL)
  if (!tool) throw new Error('no remember tool')
  return tool
}
